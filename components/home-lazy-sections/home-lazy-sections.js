document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupLazySections(config) {
    if (!config?.restUrl) {
      return;
    }
    ...
    const seasonSlug = config.seasonSlug || '';
    ...
    const getSectionRequestUrl = (sectionName) => {
      const baseUrl = `${config.restUrl}${sectionName}`;
    ...
    const loadSection = async (section, observer) => {
      const sectionName = config.getSectionName(section);
  ...
  if (window.aptoxHomeLazy?.restUrl) {
    setupLazySections({
  ...
  if (window.aptoxCelebreLazy?.restUrl) {
    setupLazySections({
